  ...
  <form class="layui-form" id="book"  lay-filter="example" onsubmit="return false">
    <div class="layui-form-item">
      ...
      <div class="layui-input-block"style="width: 60%;">
        <input type="text" id="bid" name="id" value="${requestScope.updateDept.id}" lay-verify="title" class="layui-input" readonly>
      </div>
    </div>
    <div class="layui-form-item">
      <label class="layui-form-label">部门名称</label>
      <div class="layui-input-block"style="width: 60%;">
        <input type="text" id="bname" name="deptName" value="${requestScope.updateDept.deptName}" lay-verify="title" class="layui-input">
      </div>
    </div>
    <div class="layui-form-item">
        ...
        <select name="leaderId">
          ...
          <c:forEach items="${requestScope.leaderList}" var="leader">
            <option value="${leader.id}"
                    <c:if test="${requestScope.updateDept.leaderId == leader.id }">selected="selected"</c:if>
...
<script>
  ...
  function push(){
    $.ajax({
      url:"DeptServlet?method=updateDept",
      type: "POST",
      data:$("form").serialize(),
